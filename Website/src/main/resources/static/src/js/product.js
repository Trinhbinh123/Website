...
const listThuongHieu = document.getElementById("listThuongHieu");
...
const listLoaiDe = document.getElementById("listLoaiDe");

function loadingShopPage(){
    const menuItems = document.querySelectorAll('.menu-list-item');
    menuItems.forEach(item => {
        item.classList.remove('active');
        const link = item.querySelector('a');
        if (link && link.textContent.trim() === 'Sản phẩm') {
            item.classList.add('active');
        }
    });
    $.ajax({
        url : `${api}/listThocTinhSP`,
        method : "GET",
        success : function (data){
            data.thuongHieus.forEach(item => {
                listThuongHieu.innerHTML += `<li class="filter-item">
                                                <label class="filter-label">
                                                    <input type="checkbox" onclick="filter(0,'load')" value="${item.id}"/>
                                                    <span class="filter-checkbox rounded me-2"></span>
                                                    <span class="filter-text">${item.tenThuongHieu}</span>
                                                </label>
                                            </li>`;
            })
            data.chatLieus.forEach(item => {
                listChatLieu.innerHTML += `<li class="filter-item">
                                                <label class="filter-label">
                                                    <input type="checkbox" onclick="filter(0,'load')" value="${item.id}"/>
                                                    <span class="filter-checkbox rounded me-2"></span>
                                                    <span class="filter-text">${item.tenchatlieu}</span>
                                                </label>
                                            </li>`;
            })
            data.loaiDes.forEach(item => {
                listLoaiDe.innerHTML += `<li class="filter-item">
                                                <label class="filter-label">
                                                    <input type="checkbox" onclick="filter(0,'load')" value="${item.id}"/>
                                                    <span class="filter-checkbox rounded me-2"></span>
                                                    <span class="filter-text">${item.tenloaide}</span>
                                                </label>
                                            </li>`;
            })
            data.mauSacs.forEach(item => {
                listMauSac.innerHTML += `<li class="filter-item">
                                                <label class="filter-label">
                                                    <input type="checkbox" onclick="filter(0,'load')" value="${item.id}"/>
                                                    <span class="filter-checkbox rounded me-2"></span>
                                                    <span class="filter-text">${item.tenmausac}</span>
                                                </label>
                                            </li>`;
            })
            data.sizes.forEach(item => {
                listSize.innerHTML += `<li class="filter-item">
                                                <label class="filter-label">
                                                    <input type="checkbox" onclick="filter(0,'load')" value="${item.id}"/>
                                                    <span class="filter-checkbox rounded me-2"></span>
                                                    <span class="filter-text">${item.ten_size}</span>
                                                </label>
                                            </li>`;
            })
        },
...
let totalPage = 0;
let currentPage = 1;
function filter(page, status){
    const color = getCheckbox('listMauSac');
    const size = getCheckbox('listSize');
    const chatLieu = getCheckbox("listChatLieu");
    const loadDe = getCheckbox('listLoaiDe');
    const thuongHieu = getCheckbox('listThuongHieu');
    ...
    $.ajax({
        url : `${api}/allSP?page=${page}`,
        method : "POST",
        contentType: "application/json",
        data: JSON.stringify(thuocTiinhSPResponse),
        success : function (data){
            console.log(data)
            ...
            data.forEach(item => {
                let giamGia = '';
                item.giaBan = item.giaBan.toLocaleString();
                if(item.giamGia <= 100 && item.giamGia > 0){
                    giamGia = "-" + item.giamGia + "%"
                }
                else {
                    giamGia = "-" + item.giamGia.toLocaleString() + " VND"
                }
                listProduct.innerHTML +=`<div class="col-lg-4 col-md-6 col-6" data-aos="fade-up" data-aos-duration="700">
                                        <div class="product-card">
                                            <div class="product-card-img">
                                                <a class="hover-switch" href="#" onclick="detailPage(${item.id}, event)">
                                                    <img class="secondary-img" height="270px" width="300px" src="${item.sanPhamChiTiets[0]?.anh_spct}"
                                                        alt="product-img">
                                                    <img class="primary-img" height="270px" width="300px" src="${item.sanPhamChiTiets.length > 1 && item.sanPhamChiTiets[1].anh_spct !== null ? item.sanPhamChiTiets[1].anh_spct : item.sanPhamChiTiets[0].anh_spct}"
                                                        alt="product-img">
                                                </a>

                                                <div class="product-badge" id="reduce-${item.id}">
                                                ...
                                                <div
                                                    ...
                                                    <a href="#quickview-modal" onclick="quickView(${item.id})" class="action-card action-quickview"
        ...
        complete : function (){
            ...
            $.ajax({
                url : `${api}/allSP?sizePage=99999999`,
                method : "POST",
                contentType: "application/json",
                data: JSON.stringify(thuocTiinhSPResponse),
                success : function (data){
                    document.getElementById("qtySP").innerText = "(" + data.length + ")";

                    totalPage = Math.ceil(data.length/12);
                    renderPagination(currentPage, totalPage, 3);
                },
                error : function (error){
                    console.log(error)
                }
...
function quickView(idSP){
    $.ajax({
        url : `${api}/quickView/${idSP}`,
        method : "GET",
        success : function (data){
            let colorData = [];
            let sizeData = [];

            data.giaBan = data.giaBan.toLocaleString();
            let img = ``;
            let imgThumbnail = ``;
            let color = ``;
            let size = ``;
            data.sanPhamChiTiets.forEach(item => {
                img += `<div class="img-large-wrapper">
                                                <img src="${item.anh_spct}" alt="img">
                                            </div>`;
                imgThumbnail += `<div>
                ...
                colorData.push(item.mauSac);
                sizeData.push(item.size);
            })

            colorData = Array.from(new Map(colorData.map(a => [a.id, a])).values());
            sizeData = Array.from(new Map(sizeData.map(a => [a.id, a])).values());

            console.log(colorData, sizeData)

            colorData.forEach(item => {
                color += `<li class="variant-item">
                    <input type="radio" name="color" value="${item.id}" id="color-${item.id}">
                    <label class="variant-label" id="label-color-${item.id}" for="color-${item.id}">${item.tenmausac}</label>
                </li>`;
            });

            sizeData.forEach(item => {
                size += `<li class="variant-item">
                    <input type="radio" name="size" value="${item.id}" id="size-${item.id}">
                    <label class="variant-label" id="label-size-${item.id}" for="size-${item.id}">${item.ten_size}</label>
                </li>`;
            });

            document.getElementById("ok").innerHTML = `<div class="row" id="modal-quickView">
                            <div class="col-lg-6 col-md-12 col-12">
                                <div class="product-gallery product-gallery-vertical d-flex">
                                    <div class="product-img-large">
                                        <div class="qv-large-slider img-large-slider common-slider" id="img-large">
                                            ${img}
                                        </div>
                                    </div>
                                    <div class="product-img-thumb">
                                        <div class="qv-thumb-slider img-thumb-slider common-slider" id="img-thumb">
                                            ${imgThumbnail}
                                        </div>
                            ...
                            <div class="col-lg-6 col-md-12 col-12">
                                <div class="product-details ps-lg-4">
                                    <h2 class="product-title mb-3">${data.tenSP}</h2>
                                    ...
                                    <div class="product-price-wrapper mb-4">
                                        <span class="product-price regular-price" id="newPrice">${data.giaBan}</span>
                                        <del class="product-price compare-price ms-2" id="price"></del>
                                    </div>
                                    <div class="product-sku product-meta mb-1">
                                        <strong class="label" id="kho">Kho: ---</strong>
                                    </div>

                                    <div class="product-variant-wrapper">
                                        <div class="product-variant product-variant-other">
                                            <strong class="label mb-1 d-block">Màu sắc:</strong>
                                    ...
                                    <div class="misc d-flex align-items-end justify-content-between mt-4">
                                        ...
                                        <div class="product-form-buttons d-flex align-items-center justify-content-between mt-4">
                                            <button type="submit" class="position-relative btn-atc btn-add-to-cart loader" onclick="addToCart(${data.id})">THÊM VÀO GIỎ HÀNG</button>
                                        ...
                                        <div class="buy-it-now-btn mt-2">
                                            <button type="submit" class="position-relative btn-atc btn-buyit-now" onclick="shopNow(${data.id})">MUA NGAY</button>
                                        </div>
                ...
                radio.addEventListener('click', function () {
                    ...
                    if (!isSelected) {
                        ...
                        document.querySelectorAll('input[name="size"]').forEach(input => {
                            const label = document.getElementById("label-size-" + parseInt(input.value, 10));
            ...
            document.querySelectorAll('input[name="size"]').forEach(radio => {
                radio.addEventListener('click', function () {
                    ...
                    if (!isSelected) {
                        ...
                        this.nextElementSibling.classList.add('selected'); // Thêm lớp selected
                        reloadPrice(idSP);
                        reloadThuocTinh(idSP, "size");
                    }else {
                        document.querySelectorAll('input[name="color"]').forEach(input => {
                            const label = document.getElementById("label-color-" + parseInt(input.value, 10));
                            input.disabled = false;
                            label.style.opacity = '1';
                        });
                    }
            ...
            $('#img-thumb').slick({
                slidesToShow: 5, // Số ảnh hiển thị
                slidesToScroll: 1,
                dots: false,
                ...
                verticalSwiping: false, // Vuốt theo chiều dọc
                prevArrow: '<button type="button" class="slick-prev">↑</button>', // Nút Previous
...
function reloadPrice(idSP){
    ...
    if(idColor !== null && idSize !== null){
        $.ajax({
            url : `${api}/loadPriceForProduct/${idSP}?idColor=${idColor}&idSize=${idSize}`,
            method : "GET",
            success : function (data){
                console.log(data)
                ...
                kho.innerText = "Kho: " + data.kho.toLocaleString();
                if(data.giaMoi !== 0){
                    newPrice.innerText = data.giaMoi.toLocaleString() + " VND";
                    price.innerText = data.giaCu.toLocaleString() + " VND";
                }else {
                    newPrice.innerText = data.giaCu.toLocaleString() + " VND";
                    price.innerText = "";
                }
...
function addToCart(idSP){
    ...
    $.ajax({
        ...
        error : function (error){
            showToast("Hãy chọn đủ size và màu sản phẩm", "warning");
            console.log(error)
        }
    })
}
    ...
    $.ajax({
        ...
        success : function (data) {
            ...
            if(data[0].id === -1){
                ...
                showToast("Sản phẩm không còn đủ hàng");
            }else {
...
function reloadThuocTinh(idSP, target){
    let url = ``;
    const check = checkRadio(target);
    console.log(check)
    if (target === 'color'){
        url = `${api}/checkTT/${idSP}?idColor=${check}`;
    }else {
        url = `${api}/checkTT/${idSP}?idSize=${check}`;
    }
    $.ajax({
        url : url,
        method : "GET",
        success : function (data) {
            if(target === 'color'){
                data.forEach(item => {
                    ...
                    data.forEach(item => {
                        const input = document.querySelector(`input[name="size"][value="${item}"]`);
                        const label = document.getElementById("label-size-" + parseInt(item, 10));
                        if (input) {
                            input.disabled = false;
                            label.style.opacity = '1';
                        }
                ...
                data.forEach(item => {
                    ...
                    data.forEach(item => {
                        const input = document.querySelector(`input[name="color"][value="${item}"]`);
                        const label = document.getElementById("label-color-" + parseInt(item, 10));
                        if (input) {
                            input.disabled = false;
                            label.style.opacity = '1';
                        }
...
function getDisplayedPages(currentPage, totalPages, maxPages) {
    const pages = [];
...
function renderPagination(currentPage, totalPages, maxPages) {
    const displayedPages = getDisplayedPages(currentPage, totalPages, maxPages);
    ...
    paginationContainer.innerHTML += `
        <li class="item ${prevDisabled}">
            <a class="link" href="#" data-page="${currentPage > 1 ? currentPage - 1 : 1}">
    ...
    displayedPages.forEach(page => {
        if (page === "...") {
            paginationContainer.innerHTML += `<li class="item"><span class="link dots">...</span></li>`;
        } else {
            const activeClass = page === currentPage ? 'active' : '';
            paginationContainer.innerHTML += `
                <li class="item ${activeClass}">
                    <a class="link" data-page="${page}">${page}</a>
                </li>
            `;
        }
    });
    ...
    paginationContainer.innerHTML += `
        <li class="item ${nextDisabled}">
            <a class="link" href="#" data-page="${currentPage < totalPages ? currentPage + 1 : currentPage}">
                <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"
                    viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
...
document.addEventListener('click', function (e) {
    ...
    if (link) {
        ...
        if (page) {
            ...
            filter(page - 1, 'noLoad');
            renderPagination(newPage, totalPage, 3);

            const parentItem = link.parentElement; // Lấy <li> chứa link
...
function checkRadio(name) {
    let selectedRadio = document.querySelector(`input[name="${name}"]:checked`);
    if (selectedRadio) {
