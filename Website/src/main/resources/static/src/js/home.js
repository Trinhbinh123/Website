...
const listTopNewSP = document.getElementById("listTopNewSP");

function loading() {
    $.ajax({
        url: `${api}/top?quantity=8`,
        method: "GET",
...
function renderTopSP(item) {
    ...
    let reduce = `<div class="product-badge" id="reduce-${item.id}">
...
function quickView(idSP) {
    $.ajax({
        ...
        success: function (data) {
            ...
            data.giaBan = data.giaBan.toLocaleString() + " VND";
            let img = ``;
            let imgThumbnail = ``;
            let color = ``;
            let size = ``;
...
function renderSPBanChay() {
    $.ajax({
        url: `${api}/topSPBanChay`,
        method: "GET",
        success: function (data) {
            let row = ``;
            data.forEach(item => {
                row += renderTopSP(item);
            })
            const listSPBanChay = document.getElementById("listSPBanChay");
            listSPBanChay.innerHTML = `<div class="common-slider" id="common">
                            ${row}
                        </div>
                        <div class="activate-arrows show-arrows-always article-arrows arrows-white"></div>`
            // Wait for the content to be rendered before initializing Slick
            setTimeout(function () {
                $('#common').slick({
                    slidesToShow: 4,
                    slidesToScroll: 1,
...
//
// // Lặp qua từng mục menu và gắn sự kiện click
